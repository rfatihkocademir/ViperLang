...
| `VCP004` | stdlib module missing | inspect `VIPER_STD_PATH` / installed stdlib |
| `VCP007` | package module missing at compile time | run `viper pkg install` or fix import |
| `VCP012` | unknown type annotation | inspect type name/import first |
| `VCP013` | compile-time type mismatch | inspect declared type vs inferred value |
| `VCP014` | compile-time effect mismatch | add missing effect to `@effect(...)` |
| `VCP042` | direct function arity mismatch | fix the call-site arg count before reading more code |
| `VCP043` | direct function param mismatch | inspect the typed function contract and argument expression |
| `VCP044` | duplicate ABI params field | keep a single `params=` field in `.vabi` |
| `VCP045` | invalid ABI params field | rebuild or fix malformed typed ABI contract |
| `VCP027` | undefined variable | inspect local/global/imported symbol scope |
| `VCP030` | unknown namespaced callable | inspect alias import and exported member |
| `VCP032` | unknown callable | inspect function/native name before reading more code |
| `VAS001` | async epoll init failed | inspect OS/fd environment, not user source first |
| `VME001` | nursery alloc failed | inspect process memory availability |
...
- `VAS...`: async I/O infrastructure failure
